...
export function SubjectsEdit () {
    ...
    const { currentExaPaper_subjects, eidtModalHeight, currentExaPaper } = state;
    ...
    const editSubject = async () => {
        try {
            ...
            const params = { eid: currentExaPaper.id };
            let error = '';
            ...
            actionHandler('getSubjectsList', currentExaPaper.id);
            setloadingLength(0);
        } catch (e) {
            // console.log(e);
            Modal.error({
        ...
        switch (editType) {
            case 'delete':
                Modal.confirm({
                    ...
                    async onOk () {
                        try {
                            await deleteSubjectApi(currentExaPaper.id, id);
                            actionHandler('getSubjectsList', currentExaPaper.id);
                        } catch (e) {
                        }
                    }
    ...
    return <div className={'g-editSubjectsWrap'}>
        <div className={'editsubject-header'}>
            <h2>{currentExaPaper.name}</h2>
            <div className={'labels'}>{(currentExaPaper?.labels ? currentExaPaper.labels.split(',') : []).map((l, index) => {
